...
PARSER.add_argument(
    "--architecture", action='store', help='The package architecture to use')
PARSER.add_argument(
    "--packages_dict",
    action='store',
    type=json.loads,
...
def download_deb(mirrorlist, sha256_hash, deb_path, distro_type):
    """Download a deb file from a list of mirrors"""
    success = False
    for mirror in mirrorlist:
        if distro_type == "debian":
            url = urljoin(mirror, "/debian/")
        elif distro_type == "ubuntu":
            url = urljoin(mirror, "/ubuntu/")
        url = urljoin(url, deb_path)
    ...
    for package in args.packages_dict:
        ...
        if debian:
            filename = download_deb(args.mirrors_list,
                                    ...
                                    args.packages_dict[package], "debian")
        elif ubuntu:
            filename = download_deb(args.mirrors_list,
                                    args.packages_sha256_dict[package],
                                    args.packages_dict[package], "ubuntu")
