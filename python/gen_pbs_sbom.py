...
def is_runtime(name, entry):
    return name.startswith("cpython") or name in ("pip", "setuptools") or bool(entry.get("library_names"))
        ...
        doc["packages"].append(
            {
                "name": name,
                "SPDXID": pid,
                "versionInfo": str(entry.get("version", "")),
                "downloadLocation": entry.get("url", "NOASSERTION"),
                "licenseConcluded": "NOASSERTION",
                "licenseDeclared": declared(entry),
