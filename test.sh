...
bazel build --curses=no --action_env DOTNET_CLI_HOME=/tmp  //experimental/dotnet/...
# Run all tests not tagged as "manual"
# bazel test --curses=no --test_output=errors --test_timeout=900 //...
# bazel test --curses=no --test_output=errors --test_timeout=900 --action_env DOTNET_CLI_HOME=/tmp //experimental/dotnet/...
# Run all tests tagged with "amd64"
# bazel test --curses=no --test_output=errors --test_timeout=900 $(bazel query 'attr("tags", "amd64", "//...")')
bazel test --curses=no --test_output=errors --test_timeout=900 --action_env DOTNET_CLI_HOME=/tmp $(bazel query 'attr("tags", "amd64", "//experimental/dotnet/...")')
bazel test --curses=no --test_output=errors --test_timeout=900 --action_env DOTNET_CLI_HOME=/tmp $(bazel query 'attr("tags", "arm64", "//experimental/dotnet/...")')
